    ...
    <div class="max-w-7xl mx-auto">
                ...
                <div class="group relative bg-white rounded-[3rem] overflow-hidden border border-slate-100 hover:shadow-2xl hover:shadow-brandblue/5 transition-all duration-700">
                    ...
                    @if(!$transport->available)
                        ...
                        <div class="absolute top-8 right-8 z-50">
                            <span class="px-3 py-1 bg-red-500 text-white text-[9px] font-black uppercase tracking-widest rounded-full shadow-lg">{{ __('Not Available') }}</span>
                        </div>
                    @endif
                    ...
                    <div class="h-80 overflow-hidden relative">
                        ...
                        <div class="absolute bottom-6 left-8 flex items-center gap-6 text-white/80">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[10px] font-bold uppercase tracking-wider">{{ $transport->capacity }} {{ __('Persons') }}</span>
                            </div>
                            @if($transport->available)
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[10px] font-bold uppercase tracking-wider">{{ $transport->units_left }} {{ __('Units Left') }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <!-- Content -->
                    <div class="p-10">
                        <h3 class="text-2xl font-black text-brandblue uppercase italic mb-4 leading-none group-hover:text-skyblue transition">{{ __($transport->name) }}</h3>
                        <p class="text-sm text-slate-500 font-medium line-clamp-2 leading-relaxed mb-8">{{ __($transport->description) }}</p>
                        
                        <div class="flex items-center justify-between pt-8 border-t border-slate-50 relative z-30 pointer-events-none">
                            <div>
                                <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-1">{{ __('Starting from') }}</p>
                                <p class="text-xl font-black text-brandblue italic">IDR {{ number_format($transport->price_barelang, 0, ',', '.') }}</p>
                            </div>
                            ...
                            @else
                                <span class="text-[9px] font-black text-red-400 uppercase tracking-widest italic">{{ __('Fully Booked') }}</span>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        <div class="grid md:grid-cols-3 gap-12 mt-32">
            <div class="p-10 bg-white rounded-[2.5rem] border border-slate-50 group hover:border-skyblue transition-all">
                ...
                <h4 class="text-xs font-black text-brandblue uppercase tracking-widest mb-4">{{ __('Five Star Drivers') }}</h4>
                <p class="text-[10px] text-slate-500 font-medium leading-relaxed">{{ __('Trained drivers who understand Batam routes perfectly, ensuring your punctuality and safety.') }}</p>
            </div>
