...
<head>
    ...
    <title>Admin Dashboard - Nusajaya Travel</title>
        ...
        <aside class="w-72 glass-sidebar flex-shrink-0 hidden md:flex flex-col z-50">
            <div class="p-8">
                <div class="flex items-center space-x-3">
                    <div>
                        <h1 class="text-lg font-black tracking-tighter text-slate-900 uppercase italic">Nusa Jaya Indofast T&T</h1>
                        <p class="text-[10px] font-bold text-sky-500 uppercase tracking-widest -mt-1">Admin Panel System</p>
                    </div>
                </div>
            </div>

            <nav class="mt-8 px-6 space-y-3 flex-1 overflow-y-auto">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-4 mb-4">Main Menu</p>
                ...
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-4 mb-4">Management</p>
            ...
            <div class="p-8 border-t border-slate-50">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button class="flex items-center w-full px-6 py-4 text-sm font-black text-red-500 bg-red-50 rounded-2xl hover:bg-red-500 hover:text-white transition-all duration-300 shadow-sm shadow-red-100 group">
                        <i data-lucide="log-out" class="w-5 h-5 mr-4 transition-transform group-hover:-translate-x-1"></i>
                        Logout System
                    </button>
                </form>
            </div>
        ...
        <main class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <header class="h-20 bg-white/50 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-10 z-40">
                <div class="flex items-center bg-slate-100 px-4 py-2 rounded-2xl w-96 border border-slate-200/50">
                    ...
                    <input type="text" placeholder="Search bookings, fleets, or packages..." class="bg-transparent border-none outline-none text-sm w-full text-slate-600 placeholder:text-slate-400">
                </div>
                    ...
                    <div class="relative group pl-6 border-l border-slate-100">
                        ...
                        <div class="absolute right-0 mt-4 w-64 bg-white rounded-[2rem] shadow-2xl shadow-slate-200 border border-slate-50 py-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                            <div class="px-6 py-4 border-b border-slate-50 mb-2">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Account System</p>
                            </div>
