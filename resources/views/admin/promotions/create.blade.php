...
<div class="mb-8">
    ...
    <h2 class="text-2xl font-bold text-slate-800">{{ __('Add New Promotion') }}</h2>
    <p class="text-slate-500 text-sm">{{ __('Create a new promotional offer for the website.') }}</p>
</div>
        ...
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="title" id="title" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="{{ __('Example: Batam Weekend Getaway') }}" required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="badge" id="badge" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="{{ __('Example: FLASH SALE') }}">
                </div>

                <!-- Link -->
                <div id="manual_link_wrapper">
                    <label for="link" class="block text-sm font-bold text-slate-700 mb-2">{{ __('Manual Link (Optional)') }}</label>
                    <input type="text" name="link" id="link" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="{{ __('Example: /tours') }}">
                ...
                <div>
                    ...
                    <input type="text" name="link_text" id="link_text" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="{{ __('Example: Book Now') }}">
                </div>

                <!-- Expires At -->
                <div>
                    <label for="expires_at" class="block text-sm font-bold text-slate-700 mb-2">{{ __('Expiry Date (Optional)') }}</label>
                    <input type="datetime-local" name="expires_at" id="expires_at" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                </div>

                <!-- Is Active -->
                <div class="flex items-center space-x-3 pt-8">
                    <input type="checkbox" name="is_active" id="is_active" value="1" checked class="w-5 h-5 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
                ...
                <div class="col-span-2">
                    ...
                    <p class="text-[10px] text-slate-400 mt-1">{{ __('Recommended size: 1200x600 for banners or 400x400 for cards.') }}</p>
                </div>

                <!-- Description -->
                <div class="col-span-2">
                    <label for="description" class="block text-sm font-bold text-slate-700 mb-2">{{ __('Description') }}</label>
                    <textarea name="description" id="description" rows="4" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="{{ __('Describe the promotion details...') }}"></textarea>
                </div>
            </div>
        </div>
