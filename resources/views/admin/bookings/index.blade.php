    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div class="flex items-center gap-3">
            <div class="flex items-center gap-2 px-5 py-3 bg-amber-50 dark:bg-amber-950/20 rounded-2xl border border-amber-100 dark:border-amber-900/50">
                ...
                <span class="text-xs font-black text-amber-600 dark:text-amber-400">{{ $bookings->total() > 0 ? $bookings->where('status','pending')->count() : 0 }} {{ __('Pending') }}</span>
            </div>
            <div class="flex items-center gap-2 px-5 py-3 bg-emerald-50 dark:bg-emerald-950/20 rounded-2xl border border-emerald-100 dark:border-emerald-900/50">
                ...
                <span class="text-xs font-black text-emerald-600 dark:text-emerald-400">{{ $bookings->total() > 0 ? $bookings->where('status','paid')->count() : 0 }} {{ __('Paid') }}</span>
            </div>
            <div class="flex items-center gap-2 px-5 py-3 bg-red-50 dark:bg-red-950/20 rounded-2xl border border-red-100 dark:border-red-900/50">
                ...
                <span class="text-xs font-black text-red-500 dark:text-red-400">{{ $bookings->total() > 0 ? $bookings->where('status','cancelled')->count() : 0 }} {{ __('Cancelled') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        @php
            $statusColor = match($booking->status) {
                'paid'      => ['bg' => 'bg-emerald-50 dark:bg-emerald-950/30', 'text' => 'text-emerald-600 dark:text-emerald-400', 'border' => 'border-emerald-200 dark:border-emerald-800', 'dot' => 'bg-emerald-400'],
                'pending'   => ['bg' => 'bg-amber-50 dark:bg-amber-950/30',   'text' => 'text-amber-600 dark:text-amber-400',   'border' => 'border-amber-200 dark:border-amber-800',   'dot' => 'bg-amber-400 animate-pulse'],
                'cancelled' => ['bg' => 'bg-red-50 dark:bg-red-950/30',     'text' => 'text-red-500 dark:text-red-400',     'border' => 'border-red-200 dark:border-red-800',     'dot' => 'bg-red-400'],
                'completed' => ['bg' => 'bg-blue-50 dark:bg-blue-950/30',    'text' => 'text-blue-600 dark:text-blue-400',    'border' => 'border-blue-200 dark:border-blue-800',    'dot' => 'bg-blue-400'],
                default     => ['bg' => 'bg-slate-50 dark:bg-slate-800/50',   'text' => 'text-slate-500 dark:text-slate-400',   'border' => 'border-slate-200 dark:border-slate-700',   'dot' => 'bg-slate-400'],
            };
        @endphp

        <div class="relative group">
            <a href="{{ route('admin.bookings.show', $booking) }}" class="block bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl dark:hover:shadow-black/50 hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
                    ...
                    <div class="flex-1 p-8">
                        <div class="flex flex-col lg:flex-row lg:items-center gap-6">
                            ...
                            <div class="flex items-center gap-4 lg:w-56 shrink-0">
                                <div class="w-14 h-14 rounded-2xl overflow-hidden bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-blue-600 dark:text-blue-400 font-black text-lg shadow-sm shrink-0">
                                ...
                                <div>
                                    <p class="text-sm font-black text-slate-800 dark:text-white leading-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $booking->user->name }}</p>
                                    <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-0.5 leading-tight">{{ $booking->user->email }}</p>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex flex-wrap gap-x-4 gap-y-1">
                                    ...
                                    <span class="flex items-center gap-1 text-[10px] font-bold text-slate-500 dark:text-slate-400">
