...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
    <div>
        <h2 class="text-3xl font-black text-slate-800 dark:text-white uppercase italic tracking-tight">{{ __('Edit Vehicle Unit') }}</h2>
        <p class="text-slate-500 text-sm mt-2 font-medium">{{ __('Update information for this physical car.') }}</p>
    </div>
...
<div class="max-w-3xl">
    <form action="{{ route('admin.vehicles.update', $vehicle->id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-6">
                    ...
                    <div class="relative">
                        <select name="armada_id" id="armada_id" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 outline-none transition-all font-bold text-slate-700 dark:text-slate-200 appearance-none cursor-pointer" required>
                            @foreach($armadas as $armada)
                                <option value="{{ $armada->id }}" {{ $vehicle->armada_id == $armada->id ? 'selected' : '' }}>{{ $armada->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="plate_number" id="plate_number" value="{{ $vehicle->plate_number }}" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 outline-none transition-all font-bold text-slate-700 dark:text-slate-200 uppercase" required>
                        @error('plate_number')
                            <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="mirror_number" id="mirror_number" value="{{ $vehicle->mirror_number }}" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 outline-none transition-all font-bold text-slate-700 dark:text-slate-200">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="status" id="status" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 outline-none transition-all font-bold text-slate-700 dark:text-slate-200 appearance-none cursor-pointer" required>
                            <option value="active" {{ $vehicle->status === 'active' ? 'selected' : '' }}>Active (Ready)</option>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 outline-none transition-all font-bold text-slate-700 dark:text-slate-200 resize-none">{{ $vehicle->notes }}</textarea>
                </div>
            </div>
