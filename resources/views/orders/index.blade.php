    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
            <div class="relative">
                ...
                <span class="text-skyblue font-black uppercase tracking-[0.4em] text-xs mb-3 block">Traveler Concierge</span>
            ...
            <div class="flex items-center gap-4">
                @if(count($orders) > 0)
                    <form action="{{ route('orders.reset') }}" method="POST"
                          data-confirm="This will permanently erase your entire booking history. This cannot be undone."
                          data-confirm-title="Clear Order History"
                          data-confirm-ok="Yes, Clear All">
                        @csrf
                    ...
                    <div class="p-8 md:p-10">
                        <div class="flex flex-col lg:flex-row lg:items-center gap-10">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 flex-1 gap-8">
                                <div>
                                    <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 italic">Travel Date</p>
                                    <p class="text-sm font-black text-slate-700">{{ date('d M Y', strtotime($order->travel_date)) }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 italic">Party Size</p>
                                    <p class="text-sm font-black text-slate-700">{{ $order->guests }} {{ $order->guests > 1 ? 'Persons' : 'Person' }}</p>
                                </div>
                                <div class="col-span-2 md:col-span-1">
                                    <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 italic">Total Investment</p>
                            ...
                            <div class="p-12">
                                <div class="flex justify-between items-start mb-10">
                                    <div class="relative">
                                        ...
                                        <p class="text-[10px] font-black text-brandblue">{{ $order->order_number }}</p>
                                    </div>
                                ...
                                <div class="grid md:grid-cols-2 gap-10">
                                    <div class="space-y-8">
                                        <div class="p-8 bg-slate-50 rounded-[2rem] border border-slate-100">
                                            <div class="space-y-6">
                                                <div>
                                                    <p class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 italic">Full Service</p>
                                                ...
                                                <div class="flex justify-between border-t border-slate-200/50 pt-4">
                                                    <div>
                                                        <p class="text-[11px] text-slate-400 font-black uppercase tracking-[0.2em] mb-1 italic">Date</p>
                                                        <p class="text-sm font-black text-slate-600">{{ date('d M Y', strtotime($order->travel_date)) }}</p>
                                                    </div>
                                                    <div class="text-right">
                                                        <p class="text-[11px] text-slate-400 font-black uppercase tracking-[0.2em] mb-1 italic">Investment</p>
                                        ...
                                        @if($order->status == 'pending')
                                            <div class="bg-amber-50/50 border border-amber-100 p-8 rounded-[2.5rem]">
                                                ...
                                                <form action="{{ route('orders.cancel', $order->id) }}" method="POST"
                                                      data-confirm="Release this booking? This action cannot be undone."
        ...
        <div class="mt-20 group relative overflow-hidden bg-white p-10 rounded-[3rem] border border-slate-100 flex flex-col md:flex-row items-center gap-10 shadow-sm hover:shadow-xl transition-all duration-700">
            ...
            <div class="flex-1 text-center md:text-left">
                ...
                <p class="text-xs text-slate-400 font-medium leading-relaxed italic">
                    ...
                    <a href="mailto:transportation@nusajaya168.com" class="text-skyblue font-black hover:underline">transportation@nusajaya168.com</a>.
                </p>
            </div>
            <div class="shrink-0 relative z-10">
                <a href="mailto:transportation@nusajaya168.com" class="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-skyblue transition-all duration-500 shadow-xl shadow-slate-900/10">
                    Contact Specialist
                </a>
            </div>
        </div>
